...
export default function LiffRulesPage() {
  ...
  const [newCategory, setNewCategory] = useState("carpool");
  ...
  const [editCategory, setEditCategory] = useState("carpool");
  ...
  const handleCreateRule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || "注意事項の登録に失敗しました");
      }
      ...
      setSubmitError(err.message || "注意事項の登録に失敗しました");
    } finally {
  ...
  const handleUpdateRule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = (await res.json()) as { success: boolean; error?: string };
      if (!data.success) {
        throw new Error(data.error || "注意事項の更新に失敗しました");
      }
      ...
      setSubmitError(err.message || "注意事項の更新に失敗しました");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeleteRule = async () => {
    ...
    if (!confirm(`この注意事項（${editingRule.title.slice(0, 20)}...）を削除してもよろしいですか？`)) return;
    ...
    try {
      ...
      const data = (await res.json()) as { success: boolean; error?: string };
      if (!data.success) {
        throw new Error(data.error || "注意事項の削除に失敗しました");
      }
      ...
      setSubmitError(err.message || "注意事項の削除に失敗しました");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="p-4 space-y-5">
        ...
        <LiffPageHeader
          ...
          showBack
          shareData={{
            title: `【注意事項】${currentTeam?.name || "チーム"} 活動ガイドライン一覧`,
            text: `配車マナー・当番・用具規定・熱中症対策などの注意事項はこちらから確認できます`,
          }}
        />
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="キーワードで注意事項を検索 (例: チャイルドシート、氷嚢...)"
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2">
            ...
            <span className="text-xs font-bold">注意事項を読み込み中...</span>
          </div>
                ...
                >
                  ...
                  >
                    <div className="flex items-start gap-2.5 min-w-0 flex-1">
                      <span className="w-5 h-5 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 flex items-center justify-center text-xs font-black shrink-0 mt-0.5">
                        ⚠️
                      </span>
          ...
          >
            <div className="flex items-center justify-between border-b border-border/60 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black text-foreground">新しい注意事項の登録</h3>
            ...
            <form onSubmit={handleCreateRule} className="space-y-4 text-xs font-bold">
              ...
              {/* タイトル */}
              <div className="space-y-1.5">
                <label className="text-muted-foreground">注意事項のタイトル</label>
                <input
                  type="text"
                  required
                  value={newTitle}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  placeholder="注意事項の詳しい内容や遵守事項を記入してください（箇条書き推奨）"
          ...
          >
            <div className="flex items-center justify-between border-b border-border/60 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black text-foreground">注意事項の編集</h3>
            ...
            <form onSubmit={handleUpdateRule} className="space-y-4 text-xs font-bold">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border/60">
                <button
                  ...
                  disabled={isDeleting}
                  className="px-3 py-2 rounded-xl text-destructive hover:bg-destructive/10 font-black text-xs transition-all flex items-center gap-1.5"
                >
